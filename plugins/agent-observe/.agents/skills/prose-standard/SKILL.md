---
name: prose-standard
description: Use when writing, reviewing, restoring, trimming, or auditing prose in any repository initialized with @neuravoxel/ai-eng, including deciding where documentation or comments are required across Markdown, JSDoc, code and test comments, prompts, descriptions, diagnostics, and CLI or UI strings.
---
...
Exclude third-party and vendored trees from discovery, review, and edits when the repository has them. Do not follow a symlink into them. Put exclusions after inclusion globs so a later include cannot re-admit them.
...
Treat generated catalogs, snapshots, and fixtures as derivative. Trace every consumer before editing. Edit the owning source or scenario first, then regenerate every derivative.
...
- **READMEs:** include the consumer contract: configuration, semantics, failures, limitations, and extension points. Keep durable gaps and maintainer traps, not ordinary cleanup inventories.
- **`docs/` (ai-eng consumers):** human- or package-consumer guidance only (for example testing policy and stable current-behavior references). Do not place brainstorming specs, wave execution checklists, or migration plans under `docs/` when the repo has an Agent Note tree; those belong in `.agents/notes/` (see notes README “Design drafts and checklists”).
...
- **Examples and configuration comments:** explain access limits, non-obvious wiring or load order, security stance, exceptions, and likely misuse. Do not narrate entries that the configuration already shows.
- **Prompts and visible strings:** treat wording as behavior. Prefer locale-owned copy when the product has i18n; inspect text, accessibility names, tooltips, placeholders, and format templates together.
...
2. Read the repository's documentation standard (if any) and the owning code or document before judging a passage. For calibration or unfamiliar cases, read [the distilled examples](references/examples.md).
...
6. Run the narrow relevant checks, `ai-eng verify-notes` when Agent Notes change, `git diff --check`, and behavior tests for visible strings.
