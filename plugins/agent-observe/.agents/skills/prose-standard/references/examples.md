...
**Balanced:** “Test registration and disposal at unit level, exercise the tool through the real entry path, and add an expected-output check when rendered output changes. Verify the assertion observes the external result rather than the model's report.”
...
**Balanced:** “Unit tests cover cancellation before and after publication, disposal quiescence, and provider reload. A built-entry smoke covers the real loader path; broader product round-trip coverage is deferred because the transport is process-specific.”
...
**Over-trimmed:** Omitting a process-lifetime cache that makes configuration changes require reload.
