...
打开任意 Session → **编排** Tab，选择 preset 查看组合行（只读）。画布按 [wiki/011 九层](../../wiki/011-插件分组与主要作用.md) 分带显示（仅展示有成员的层）。当查看**当前会话**正在用的 Preset 且 turn 执行中时，会按 `tool/call` 高亮对应插件（observe 仍负责业务 flow / 数据流）。点击插件节点可查看条目 ID、模块、架构层、包组、启用状态、条件、Fiber 阶段与锁定。
...
| `src/client/` | 对话 Tab + AITopoHost + Session 活动源 |
