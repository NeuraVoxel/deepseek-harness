# Agent Note: agent-observe harness frame nodes (Profile / Session / Envelope / Memory / Context)

Status: implemented

English | [中文](2026-09-08-agent-observe-harness-flow-nodes.zh.md)

## Problem

The agent-observe process view showed only the Agent Loop pipeline. Operators could not see harness concepts (Profile / Session / Envelope / Memory / Context), Client↔Host wire methods, or honest data vs control edges.

## Decision

Extend `plugins/agent-observe` flow derivation only (no `packages/` changes):
...
- Required verification: `pnpm --filter dsh-agent-observe test`.
