# Agent Note: agent-observe harness 框架节点（Profile / Session / Envelope / Memory / Context）

Status: implemented

[English](2026-09-08-agent-observe-harness-flow-nodes.md) | 中文

## Problem

agent-observe 过程视图原先只画 Agent Loop。运维看不到 harness 概念（Profile / Session / Envelope / Memory / Context）、Client↔Host 线上方法，以及诚实的数据边 vs 控制边。

## Decision

仅扩展 `plugins/agent-observe` 的 flow 推导（不改 `packages/`）：
...
- 必验：`pnpm --filter dsh-agent-observe test`。
