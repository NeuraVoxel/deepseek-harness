...
Placement: bugs → postmortems; rationale → Agent Notes; study → [.agents/learning/](../.agents/learning/README.md); procedures → cookbooks; type definitions → subsystems; package contracts → READMEs; standing orders → root `AGENTS.md` with a rationale link.
