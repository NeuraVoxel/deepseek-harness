...
Provider tokenization governs exact text and image-token input. The adapter declares per-route `imageRequestPricing`: it reproduces oldest-first image offload from durable byte lengths and prices each retained image at its projected dimensions with the published v4 vision accounting (14px patch grid, 3:1 downsampling, 384-token cap, worst-case alignment pad). This lets the token meter price image pressure before a request; reported usage remains authoritative. The adapter also declares per-route `tokenMoneyRates` from the published off-peak USD list prices (cache-miss input, cache-hit input, output) as the Host baseline; Client Turn-usage estimates multiply exact token buckets with the parallel USD/CNY tables and select peak (2×) from each attempt's settlement time in the provider's weekday UTC windows; unpublished models omit rates and the UI omits money. The panel labels the figure as an estimate. Reasoning passback carries every reasoned turn's chain of thought into later requests, while dropping over-budget images avoids paying those tokens again. Cache-read usage is reported when available. `totalTokens` is the exact `prompt_tokens + completion_tokens` aggregate and is omitted if a supplied `total_tokens` disagrees.
