...
- **Non-trivial changes MUST include an Agent Note in the same PR;** only mechanical/local edits are exempt ([scope](.agents/notes/README.md#when-to-write-one)). Plugin kit notes: [plugins/AGENTS.md](plugins/AGENTS.md). Archived notes are frozen ([archive policy](.agents/notes/README.md#archiving-and-deletion)).
